...
function mountWorkInProgressHook(): Hook {
  ...
  if (workInProgressHook === null) {
    ...
    // 将首个hook 放到当前执行的 FunctionComponent的 fiber节点的  memoizedState 属性上
...
function updateReducer<S, I, A>(
  ...
  if (baseQueue !== null) {
    ...
    do {
      ...
      // 判断优先级，优先级不够跳过此更新， 放入newBaseQueueLast中等待下次更新
      if (!isSubsetOfLanes(renderLanes, updateLane)) {
        //优先级不足。跳过这个更新。如果这是第一次
        //跳过更新，之前的更新/状态是新的基础
        //更新/状态。
        ...
        if (update.eagerReducer === reducer) {
          // 如果这个更新是个紧急更新 并且 它的 reducer 等于当前的 的 reducer 我们使用紧急跟新的值
  ...
  // are remaining.
  //交叉更新存储在一个单独的队列。我们不会的
  //在渲染过程中处理它们，但我们需要跟踪哪些车道
  //剩余。
...
function dispatchAction<S, A>(
  ...
  if (
    ...
    //这是一个渲染阶段更新。把它藏在一张懒人制作的地图里
    // queue ->链表更新。在这个渲染过程之后，我们将重新开始
    //并将隐藏的更新应用到正在工作的钩子上。
    ...
    if (
      ...
      //进入渲染阶段之前的下一个状态。如果新状态是
      //如果和目前的情况一样，我们可能会完全摆脱困境。
