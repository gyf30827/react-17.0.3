...
export function markStarvedLanesAsExpired(
  ...
  // 每次我们yield时都会调用这个函数。我们可以通过在根节点上存储最早的过期时间来进行优化。
  // 然后用它来快速退出这个函数。
  ...
  //遍历挂起的通道并检查我们是否达到了它们的过期时间。
  // 如果是这样，我们将假定更新被终止，并将其标记为过期以强制它完成。
  ...
  while (lanes > 0) {
    ...
    if (expirationTime === NoTimestamp) {
      //找到一个没有过期时间的挂起通道。如果它没有被挂起，
      // 或者被ping，则假定它是cpu绑定的。使用当前时间计算新的过期时间。
