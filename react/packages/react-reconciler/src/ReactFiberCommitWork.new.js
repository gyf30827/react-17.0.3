...
function commitBeforeMutationEffectsDeletion(deletion: Fiber) {
  // 如果能避免调用doesFiberContain()就好了，
  // 也许我们可以重新使用其中一个subtreeFlags位置来代替它?
  // 使用它来存储焦点实例位于树的哪个部分?这假设我们可以在“渲染”阶段安全地确定该实例。
    ...
    switch (finishedWork.tag) {
      ...
      case ClassComponent: {
        ...
        if (finishedWork.flags & Update) {
          // 通过current 是否为 null 来判断是 mount 还是 update
...
function commitPlacement(finishedWork: Fiber): void {
  ...
  // 我们只有插入的 顶层 fiber 但是我们需要递归它的所有children 找到所有的终端节点
  // We only have the top Fiber that was inserted but we need to recurse down its
  // children to find all the terminal nodes.
...
function commitDeletion(
  ...
  if (supportsMutation) {
    ...
    //递归删除父节点中的所有主机节点。卸载refs并在整个子树上调用componentWillUnmount()。
    unmountHostComponents(finishedRoot, current, nearestMountedAncestor);
  } else {
    // 卸载refs并在整个子树上调用componentWillUnmount()。
    // Detach refs and call componentWillUnmount() on the whole subtree.
...
// 更新ref
  ...
  // 下面的switch语句只关注放置、更新和删除 为了避免需要为每个可能的位图值添加一个case
  ...
  outer: switch (primaryFlags) {
    case Placement: {
      ...
      // 在调用componentDidMount这样的生命周期之前，
      // 清除effect标签中的“placement”，这样我们就知道它已经被插入了。
...
function ensureCorrectReturnPointer(fiber, expectedReturnFiber) {
  ...
  // 一旦我们确信它不会破坏任何东西，就删除这个赋值，通过检查上面不变式的警告日志
