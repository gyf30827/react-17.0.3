...
// 标识 root 节点的状态，completed, errored, suspended 等
...
export function scheduleUpdateOnFiber(
  ...
  if (lane === SyncLane) {
    ...
    if (
      ...
      //这是一个遗留的边缘情况。reactdom . rendered的初始加载
      // root内部的batchedupdate应该是同步的，但布局更新
      //应该延迟到批处理的结束。
...
export function isInterleavedUpdate(fiber: Fiber, lane: Lane) {
  return (
    ...
    (fiber.mode & ConcurrentMode) !== NoMode &&
    //如果这是渲染阶段的更新(例如UNSAFE_componentWillReceiveProps)，
    //那么不要把它当作一个交叉更新。这种模式是
    //伴随着警告，但我们还没有完全弃用它。我们可以
    //当deferRenderPhaseUpdateToNextBatch标志被启用时删除
...
// 使用这个函数调度一个 根 任务，每个任务只有一个根
// Use this function to schedule a task for a root. There's only one task per
// 如果一个任务已经被调度过，我们将检查以确保现有任务的优先级与根操作的下一个级别的优先级相同
// root; if a task was already scheduled, we'll check to make sure the priority
// of the existing task is the same as the priority of the next level that the
// 这个函数在每次 update 时调用， 并在退出之前调用
// root has work on. This function is called on every update, and right before
// exiting a task.
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  const existingCallbackNode = root.callbackNode;
  //检查是否有通道被其他工作所堵塞。如果是这样，就把它们标记为过期，这样我们就知道接下来要做什么了。
  ...
  //我们使用最高优先级的lane来表示回调的优先级。
  const newCallbackPriority = getHighestPriorityLane(nextLanes);

  // Check if there's an existing task. We may be able to reuse it.
  // 检查是否存在一个现有的任务。我们也许可以重复使用它。
  ...
  if (newCallbackPriority === SyncLane) {
    ...
    // 以 schedulerPriorityLevel 任务优先级 执行 performConcurrentWorkOnRoot 函数
...
//这是每个并发任务的入口点，即任何
// 可以简单的如下认为： 时间切片：每次宏任务执行
// fiber 树一小块，执行时间到就中断，调度下一个宏任务，循环此操作。
// 高优先级插队：每次任务执行时，判断是否有高优先级任务，
// 有则取消当前任务，执行高优先级任务。 简化代码：
//通过调度程序。
function performConcurrentWorkOnRoot(root, didTimeout) {
  if (enableProfilerTimer && enableProfilerNestedUpdatePhase) {
    resetNestedUpdateFlag();
  }
  // 由于我们是在一个react 事件中， 我们可以清空当前的事件的时间，下一次 update 将会重新计算一个时间
  ...
  // 在决定在那个 lanes 组 工作前， 清空所有 pending passive effects
  // Flush any pending passive effects before deciding which lanes to work on,
  // 防止它们 schedule 额外功能工作
  // in case they schedule additional work.
  const originalCallbackNode = root.callbackNode;
  const didFlushPassiveEffects = flushPassiveEffects();
  if (didFlushPassiveEffects) {
    // 处于passive effect 阶段的任务可能已经取消了当前的任务
    // Something in the passive effect phase may have canceled the current task.
    // 检查这个任务节点是否被修改
    // Check if the task node for this root was changed.
    if (root.callbackNode !== originalCallbackNode) {
      // 当前的任务被取消了，退出，不需要在此调用
      // The current task was canceled. Exit. We don't need to call
      // ensureRootIsScheduled 因为上面的检查暗示了其中之一
  ...
  //我们只是防御性地检查' didTimeout '，以解释我们仍在调查的调度程序错误。一旦调度程序中的错误被修复，
  // 我们就可以删除它，因为我们自己跟踪过期时间。
  ...
  if (exitStatus !== RootIncomplete) {
    ...
    // 现在我们有了一个完成的树，下一步就是 commit 它
    // We now have a consistent tree. The next step is either to commit it,
    // 或者 因为什么其他事情挂起，等到超时后提交
    // or, if something suspended, wait to commit it after a timeout.
    const finishedWork: Fiber = (root.current.alternate: any);
    root.finishedWork = finishedWork;
    root.finishedLanes = lanes;
    // commit 将fiber 树渲染到页面
  ...
  if (root.callbackNode === originalCallbackNode) {
    ...
    //为这个根节点调度的任务节点与当前正在执行的任务节点相同。需要返回一个延续。
...
function finishConcurrentRender(root, exitStatus, lanes) {
  switch (exitStatus) {
    ...
    case RootErrored: {
      // 我们应该已经尝试重试这个 tree 如果我们到了这个阶段，它又出错了，commit it
    ...
    case RootSuspended: {
      markRootSuspended(root, lanes);
      // 我们有一个可以接受的 等待状态 ， 我们需要计算 如果我们要立即提交它的话
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  // 根据 unitOfWork 是否有 alternate 数据决定是 mount 还是 update
  ...
  // 如果下一个节点是null 进入  complete 归 阶段
  if (next === null) {
    // If this doesn't spawn new work, complete the current work.
    completeUnitOfWork(unitOfWork);
  } else {
    // 将下一个fiber 节点复制给  workInProgress 进行下一轮循环
    workInProgress = next;
  }
...
function commitRootImpl(root, renderPriorityLevel) {
  do {
    // ' flushPassiveEffects '将在末尾调用' flushSyncUpdateQueue '，
    // 这意味着' flushPassiveEffects '有时会导致额外的被动效果。因此，我们需要在循环中不断刷新，
    // 直到没有更多挂起的效果。待办事项:如果“flushPassiveEffects”在结束时没有自动清除同步工作，
    // 可能会更好，以避免这样的风险。
  ...
  // root.current.alternate; rootFiber
  ...
  // commitRoot永远不会返回一个延续;它总是同步完成。
  // 所以我们现在可以清除这些，允许一个新的回调被安排。
  ...
  //如果有挂起的被动效果，调度一个回调来处理它们。
  // 尽早执行此操作，以便它排在提交阶段可能调度的任何其他内容之前。
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    ReactCurrentOwner.current = null;
    // 提交阶段分为几个子阶段。
    // 我们为每个阶段单独传递 effect list :
    // 所有的 mutation effects 都出现在所有 layout effects 之前
    // The commit phase is broken into several sub-phases. We do a separate pass
    // of the effect list for each phase: all mutation effects come before all
    // layout effects, and so on.
    // 第一个阶段是  before mutation 我们使用这个阶段来读取宿主树的
    // state 在  mutate 之前 也就是调用 getSnapshotBeforeUpdate 的地方
    ...
    // 下一个阶段就是  mutation 阶段，我们将要挂载 host tree
    ...
    // 正在进行的工作树现在是当前树。这必须在突变阶段之后进行，
    // 这样在componentwillumount期间之前的树仍然是当前的，但在布局阶段之前，
    // 这样在componentDidMount/Update期间完成的工作是当前的。
    // The work-in-progress tree is now the current tree. This must come after
    // the mutation phase, so that the previous tree is still current during
    // componentWillUnmount, but before the layout phase, so that the finished
    // work is current during componentDidMount/Update.
    root.current = finishedWork;
    // 下一个阶段是布局阶段，在这里我们调用在宿主树被改变后读取它的效果。
    // 这方面的惯用用例是布局，但由于遗留原因，类组件的生命周期也在这里触发。
