...
// didReceiveUpdate === false 时 即可以直接复用前一次更新的 子 Fiber 不需要新建 Fiber
...
export function reconcileChildren(
  ...
  if (current === null) {
    // 首次挂载时
    //如果这是一个还没有被渲染的新组件，我们不会通过最小化副作用来更新它的子组件集。
    // 相反，我们将在它被渲染之前将它们全部添加到子节点。这意味着我们可以通过不追踪副作用来优化这个和解过程。
    ...
    // 更新组件
    //如果当前子进程与正在进行的工作相同，则表示还没有开始对这些子进程进行任何工作。
    // 因此，我们使用克隆算法来创建所有当前子节点的副本。
    // 如果我们已经有任何进展的工作，在这一点上是无效的，所以让我们把它扔了。
...
function beginWork(
  ...
  if (current !== null) {
    ...
    if (
      ...
      // 如果 props 获取 context 改变，标记这个 fiber 完成工作
      // If props or context changed, mark the fiber as having performed work.
      // 如果稍后判断props 相等， 则可以取消设置
